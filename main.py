...
def build_grounded_prompt(kb_data: Optional[dict], question: str) -> str:
    """Builds a vision-oriented prompt combining visual inspection with optional contextual ground truth."""
    context_section = ""
    if kb_data:
        ...
        visible_features = "\n- " + "\n- ".join(kb_data.get("visible_features", []))
        
        land_use = kb_data.get("land_use_information", {})
        land_use_str = "\n- " + "\n- ".join([f"{k.replace('_', ' ').title()}: {v}" for k, v in land_use.items()])
        
        context_section = f"""
=== REFERENCE SCENARIO METADATA ===
- Scenario: {scenario}
- Geographic Setting: {location}
- Reference Summary: {description}
- Ground-Truth Features:{visible_features}
- Land-Use Profile:{land_use_str}
===================================
"""

    prompt = f"""
You are SatQuery AI, an expert Earth Observation and Satellite Image Vision Assistant.
...
1. Visually examine the provided satellite / aerial image with extreme attention to detail.
2. Directly answer the user's question based on what is VISUALLY PRESENT in the image (e.g. roads, transport vehicles/trucks, tree canopy, cleared or disturbed earth, soil excavation, water bodies, coastlines, clouds/storms, sandbars).
3. If reference metadata is provided above, use it to give accurate domain context, but ALWAYS ensure your visual findings accurately reflect what is physically visible in the image.
4. Structure your response clearly using markdown headings (e.g. `### 🛰️ Visual Analysis`), bullet points, and bold keywords for readability. Be precise, factual, and concise.
"""
    return prompt.strip()
...
def generate_gemini_analysis(
    ...
    """Sends the satellite image and vision prompt to Gemini."""
    ...
    for model_name in candidate_models:
        try:
            ...
            if response and response.text:
                print(f"[Gemini AI] Successfully generated response using model '{model_name}'.")
                return response.text
        except Exception as err:
            last_exception = err
            print(f"[Gemini Warning] Model '{model_name}' failed: {err}")
...
async def analyze_image(
    ...
    if kb_data:
        ...
        unrecognized_msg = f"""### ⚠️ Uncalibrated Image (Demo Mode)

The uploaded image (`{orig_filename}`) is not one of the pre-calibrated prototype scenarios.

To analyze **any custom satellite image** with real-time AI vision, please add your **Gemini API Key** in the `.env` file:
```env
GEMINI_API_KEY=AIzaSy...
```
Or select one of the calibrated sample scenarios from the top bar."""
        return AnalysisResponse(
            answer=unrecognized_msg,
            mode="SatQuery Knowledge Engine (Demo Mode)",
            scenario="Uncalibrated Image",
            image_id=None,
        )
